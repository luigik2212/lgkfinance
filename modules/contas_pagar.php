...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['create', 'update'], true)) {
        ...
        $data = [
            ...
            $_SESSION['user']['id'],
            !empty($_POST['usuario_pagou_id']) ? (int)$_POST['usuario_pagou_id'] : null,
            (float)$_POST['valor'],
...
<div class="card">
    <div class="card-header-row">
        <h3>Filtros</h3>
        <a class="btn-secondary" href="index.php?module=contas_pagar&show_filters=<?= $showFilters ? '0' : '1' ?>"> <?= $showFilters ? 'Esconder filtros' : 'Exibir filtros' ?> </a>
    </div>
    <?php if ($showFilters): ?>
        <form class="filters">
            <input type="hidden" name="module" value="contas_pagar">
            ...
            <select name="status"><option value="">Status</option><?php foreach(['Pendente','Pago','Vencido','Cancelado','Parcelado'] as $s): ?><option <?= $fStatus === $s ? 'selected' : '' ?>><?= $s ?></option><?php endforeach; ?></select>
            ...
            <input name="q" placeholder="Buscar" value="<?= esc($fBusca) ?>">
...
<div class="card">
    ...
    <div class="table-wrap">
        <table>
            <tr><th>Descrição</th><th>Categoria</th><th>Responsável</th><th>Venc.</th><th>Valor</th><th>Status</th><th>Ações</th></tr>
            <?php foreach ($contas as $c): $atrasada = $c['status'] !== 'Pago' && strtotime($c['data_vencimento']) < time(); ?>
                <tr class="<?= $atrasada ? 'row-danger' : '' ?>">
                    ...
                    <td class="actions">
                        <button class="btn-secondary" type="button" data-modal-target="modal-conta-pagar-edit-<?= $c['id'] ?>">Editar</button>
                        <form method="post"><input type="hidden" name="action" value="mark_paid"><input type="hidden" name="id" value="<?= $c['id'] ?>"><button class="btn-success" type="submit">Pago</button></form>
    ...
    <div class="pagination"><?php for ($i = 1; $i <= $totalPages; $i++): ?><a class="<?= $i === $page ? 'active' : '' ?>" href="index.php?module=contas_pagar&page=<?= $i ?>&show_filters=<?= $showFilters ? '1' : '0' ?>&mes=<?= urlencode($fMes) ?>&categoria=<?= urlencode($fCategoria) ?>&status=<?= urlencode($fStatus) ?>&q=<?= urlencode($fBusca) ?>&recorrente=<?= urlencode($fRec) ?>"><?= $i ?></a><?php endfor; ?></div>
</div>
...
<div class="modal" id="modal-conta-pagar-create" aria-hidden="true">
    <div class="modal-content modal-lg">
        ...
        <form method="post" enctype="multipart/form-data" class="grid-form">
            ...
            <select name="categoria_id" required><?php foreach($categorias as $cat): ?><option value="<?= $cat['id'] ?>"><?= esc($cat['nome']) ?></option><?php endforeach; ?></select>
            ...
            <input name="tipo_pagamento" placeholder="PIX, Boleto...">
            <label><input type="checkbox" name="recorrente"> Conta recorrente?</label>
            <label><input type="checkbox" name="parcelada" data-toggle-target="create-parcelas"> Conta parcelada?</label>
            <div data-toggle-id="create-parcelas" class="toggle-extra hidden"><input type="number" min="2" name="numero_parcelas" placeholder="Número de parcelas"></div>
...
<?php foreach ($contas as $c): ?>
    <div class="modal" id="modal-conta-pagar-edit-<?= $c['id'] ?>" aria-hidden="true">
        <div class="modal-content modal-lg">
            ...
            <form method="post" enctype="multipart/form-data" class="grid-form">
                ...
                <select name="categoria_id" required><?php foreach($categorias as $cat): ?><option value="<?= $cat['id'] ?>" <?= $c['categoria_id'] == $cat['id'] ? 'selected' : '' ?>><?= esc($cat['nome']) ?></option><?php endforeach; ?></select>
                ...
                <input type="date" name="data_vencimento" value="<?= esc($c['data_vencimento']) ?>" required>
                <input type="date" name="data_pagamento" value="<?= esc($c['data_pagamento']) ?>">
                <select name="status"><?php foreach($statusList as $s): ?><option <?= $c['status'] === $s ? 'selected' : '' ?>><?= $s ?></option><?php endforeach; ?></select>
                <input name="tipo_pagamento" value="<?= esc($c['tipo_pagamento']) ?>" placeholder="PIX, Boleto...">
                <label><input type="checkbox" name="recorrente" <?= $c['recorrente'] ? 'checked' : '' ?>> Conta recorrente?</label>
                <label><input type="checkbox" name="parcelada" data-toggle-target="edit-parcelas-<?= $c['id'] ?>" <?= (int)$c['numero_parcelas'] > 1 ? 'checked' : '' ?>> Conta parcelada?</label>
                <div data-toggle-id="edit-parcelas-<?= $c['id'] ?>" class="toggle-extra <?= (int)$c['numero_parcelas'] > 1 ? '' : 'hidden' ?>"><input type="number" min="2" name="numero_parcelas" value="<?= (int)$c['numero_parcelas'] ?: '' ?>" placeholder="Número de parcelas"></div>
                <input type="file" name="anexo" accept=".pdf,image/*">
